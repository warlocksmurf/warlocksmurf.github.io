...
We are given a JPG image of John Cena to investigate. Analyzing the image metadata, a secret key can be obtained for steghide to obtain the flag.
...
We are given a suspiciously formatted ciphertext to investigate. Analyzing the ciphertext on dcode, it seems that the encoding method was called `Zero-width Space (ZWSP)`.
...
We are given a Desktop screenshot to investigate. Looking at the bottom right of screenshot, the flag was cut-off due to the height.
...
We are given a suspiciously spaced ciphertext to investigate. This time it was obvious that the encoding method used here was `Whitespace programming/SNOW`. 
...
Reading the challenge name, it seems to be mentioning about DeepSound, a common steganography tool used for audio. However, it seems that the audio was password protected. So the password must be cracked first before obtaining the secret file.
