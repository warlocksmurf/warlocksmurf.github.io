...
categories: [ctf,hackerscrew]
tags: [forensics]
image: /assets/posts/crewctf2024/icon.png
---

This is a writeup for all forensics challenges from CrewCTF 2024. Another great CTF from a well-known team where L3ak managed to achieve 5th place. I am glad to finally have the opportunity to try out seal's forensics challenges after knowing him for almost a year now. I also won the best forensics writeup contest!
...
My teammate @MinatoTW managed to locate the [packet structure](https://github.com/RfidResearchGroup/proxmark3/blob/9842416f3047075355944bf118d74afb71764011/doc/new_frame_format.md#new-format) for Proxmark3 packets. Essentially, Proxmark3 packets follow a structured format, which includes a magic byte, command, length, CRC, and payload. Analyzing the packets in the PCAP, we can see the packets having magic bytes `PM3a` and `PM3b`, both indicating requests and responses respectively. According to this [manual](https://github.com/RfidResearchGroup/proxmark3/blob/9842416f3047075355944bf118d74afb71764011/include/pm3_cmd.h) on the command types, a parser was created by @MinatoTW to order the data by their block number and took the output that corresponded to each block number.
