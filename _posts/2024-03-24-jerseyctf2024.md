...
Just check the audio spectrogram.
...
We are given a corrupted jpg file. Using binwalk, it was shown that it was actually a zip file, not a jpg file.
...
After extraction, we are given a `password.txt` file and another zip file to crack.
...
So it was obvious that a dictionary attack was required. Using `7z2john`, the zip file hash can be obtained for further cracking. The password was `jimihendrix123!`.
...
We are given a password-protected zip file that has a text file of the flag and a SVG file. Remembering a CTF I've played previously, this relates to the plaintext attack with `bkcrack` since the content within the zip was compressed using the `Store` method. 
...
Reading the description, I was right, however, there is a twist. See, most blogs will mention SVG headers to have `<?xml version="1.0"`. However, according to [Mozilla](https://developer.mozilla.org/en-US/docs/Web/SVG/Tutorial/Getting_Started), SVG headers can be this too `<svg version="1.1"`. So the plaintext we were required to use was actually `<svg version="1.1"`, not the usual `<?xml version="1.0"`. I asked the author on this and he agreed its true, he wanted players to actually understand the attack rather than copy pasting codes from README.md (LMAO).
