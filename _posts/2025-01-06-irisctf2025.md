...
One writeup that stood out the most was from [@Stefan](https://stefanin.com/posts/irisctf-tracem2/), where his solve method seems the most logical to me. Since the user might have impersonated another user, the DHCP flow from the user's machine would be `DHCPREQUEST > DHCPREQUEST > DHCPRELEASE`, indicating that at least 2 IPs were assigned simultaneously to the same MAC address. So a script can be created to identify MAC addresses that have sequential DHCP requests without DHCP releases.
