...
We are given a memory dump to investigate. Analyzing the processes, several Discord processes can be identified. This suggests communication on Discord since the missing document could not be identified using filescan.
...
One thing that was pretty suspicious was that Discord spawned several processes from a single Discord parent process. Using ChatGPT on this, it suggested that Discord apparently spawned different processes to handle a specific function. One of the process seem to be responsible for handling text/images, so that is the process that should be focused on.

![ctf8](/assets/posts/tcp1pctf2024/ctf8.png)

Dumping and analyzing the process, me and my teammate @0x157 managed to identify parts of an encrypted text using the GIMP technique. However, the authors mentioned that this was not the intended method.

![ctf9](/assets/posts/tcp1pctf2024/ctf9.png)

![ctf10](/assets/posts/tcp1pctf2024/ctf10.png)

After spending several hours bruteforcing the offsets, I suddenly remembered about using the classic strings + grep. This was one of the funniest thing that happened in our CTF life no cap.

![ctf11](/assets/posts/tcp1pctf2024/ctf11.png)

So after stringings for the partial text that we obtained from GIMP, we managed to retrieve several conversations from the Discord chat. One thing that we figured out was that each message was placed right below a variable called `contentm`.

![ctf12](/assets/posts/tcp1pctf2024/ctf12.png)

My teammate managed to identify the method to fully decode it. It seems that the messages were not formatted properly since some of the special characters were used in Discord for formatting. So the actual encoded strings can be obtained by pasting the text on Discord. Example:

`F\*Z3iGdMXiI5jsgF\*G<bF)=eSG&eChGB+?cHZnOdF\*7$fHa0OfH#0akI5smdG%+<aH90UbGd46iIXN\~qHZU\|ZH#0IZHZnIfG&VIhHZeFhGBPtcH#s#pFf=tXG&VQm` will turn into:

![ctf13](/assets/posts/tcp1pctf2024/ctf13.png)

So a script can be created to decode the actual encoded strings and obtain the cleartext messages.
...
Now we have a drive link that leads us to a broken ZIP file.

![ctf14](/assets/posts/tcp1pctf2024/ctf14.png)

Here we were stuck for awhile, but our goat @MinatoTW managed to fix the ZIP file by changing the filename length, extra field length, set flags to encrypted and fix the timestamp. I'm not the greatest at ZIP forensics so I won't dive into it much.

![ctf15](/assets/posts/tcp1pctf2024/ctf15.png)

However, the ZIP was password protected. So a simple John the Ripper crack will crack it and provide the flag in a document file.
...
Dumping the notepad process, the flag can be obtained using the GIMP visualisation technique (read my other writeups to learn about this technique). 
