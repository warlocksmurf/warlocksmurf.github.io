...
This is a writeup for some misc and OSINT challenges from DeadSec CTF 2024. Another W for L3ak where we achieved 1st place 🥇! The misc challenge was interesting as it showcased a new method of cracking legacy zip files. However, the OSINT challenges were very lackluster, hope the authors improve them next year.
...
We are given a zip file and a Python script to investigate. Analyzing the Python script first, it seems that it was just a script to generate an iso file into a zip file using the flag as the password.
...
Analyzing the given zip file, the compression method of the zip file can be identified as `ZipCrypto Deflate`. This highly suggests a known plaintext attack.
...
One good tool to crack this type of zip file is [bkcrack](https://github.com/kimci86/bkcrack). However, the iso file was known to be deflated before it was compressed. Hence, a new iso file can be generated and compressed using the Python script given to analyze its content after compression. To reduce false positives, two iso files were generated and tested.

![zip1](/assets/posts/deadsecctf2024/zip1.png)

Notice how both compressed iso files have a long series of null bytes. This was due to the [nature](https://superuser.com/questions/450272/unable-to-open-iso-file-by-using-winrar) of iso files where they usually begin with a lot of null bytes. So, we can easily use the null bytes as the plaintext for bkcrack to obtain the appopriate keys.
...
After obtaining the keys, just decrypt the zip file and obtain the flag.
