...
tags: [forensics,misc,rev,osint]
image: /assets/posts/wgmyctf2024/icon.png
---

This is a writeup for some forensics, misc and OSINT challenges from Wargames.MY 2024. As promised last year, I will come back stronger for Wargames.MY CTF and the results showed progress. What made this CTF special this year was the organizers allowing no team limit for open division teams, so this gave us the opportunity to form our own "Avengers" squad to win this thing together. Fortunately enough, we managed to achieve 3rd place out of 266 teams (was a close battle between the top 3 teams). Please sponsor us Hot Bird!
...
We are given a PCAP file to investigate. According to the challenge title, this is a classic forensics challenge that involves ICMP packets to store the flag characters within their data byte.

![icmp](/assets/posts/wgmyctf2024/icmp.png)

Using a simple script, the flag can be obtained.

```python
import subprocess
import binascii

def extract_icmp_data_with_tshark(pcap_file):
    try:
        tshark_command = [
            "tshark",
            "-r", pcap_file,
...
if __name__ == "__main__":
    ...
    result = extract_icmp_data_with_tshark(pcap_file_path)
...
We are given a broken JPG image to investigate. Analyzing it, there were several bogus "meow" data in different sections of the image. 

![cat1](/assets/posts/wgmyctf2024/cat1.png)

Using a simple Python script, the JPG image can be fixed accordingly.
...
We are given a PCAP file to investigate. A stream of SMB2 encrypted packets can be identified, so we must most likely find a key to decrypt them. Here, we can easily locate NTLM authentication exchanges within the protocol.

![smb1](/assets/posts/wgmyctf2024/smb1.png)

This was similar to a CTF that I've played where we have to craft our own NTLM hash using the parameters in the authentication packets. Here is a good Youtube video about it: https://www.youtube.com/watch?v=lhhlgoMjM7o

![smb2](/assets/posts/wgmyctf2024/smb2.png)

Following the video, specific information from both the response and request packets had to be extracted for the NTLM hash. The final NTLM hash should look something like this:
...
After cracking the hash, we can use the password to decrypt the SMB2 packets and dump the SMB objects from the PCAP.

![smb4](/assets/posts/wgmyctf2024/smb4.png)

![smb5](/assets/posts/wgmyctf2024/smb5.png)

The `5cRxHmEj` file seems to be showing a series of commands to dump the credentials. So we can essentially download nanodump and pypykatz to get the flag from the log file.
...
Following the commands, the flag can be obtained.
...
We are given a memory dump and PCAP file to investigate. Analyzing the processes within the memory dump, several suspicious processes from `crypt.exe` can be identified.
...
However, it seems that there were three instances of `crypt.exe` being present in the system. Dumping the first `crypt.exe` file, we can see identify it to be a PyInstaller executable file.
...
We are given two binary files to investigate.
...
Reading more about what the files were, a user from this [forum page](https://www.zenhax.com/viewtopic.php@t=1013.html) mentioned that the UCL compressed DAT file can actually be unpacked using a tool called **quickbms** and the specific game script **tokyo_xtreme_racer.bms**.

![osint2](/assets/posts/wgmyctf2024/osint2.png)

![osint3](/assets/posts/wgmyctf2024/osint3.png)

Unpacking the file, multiple game data and texture files can be obtained from it. After countless hours (and a great sushi lunch with the boys), I managed to figure out a dumb way to supposedly obtain the flag. Since the texture files could be analyzed using tools like **GIMP**, I attempted to brute force my way to find the flag from ALL the texture files. Additionally, since the challenge description mentioned something about `Melancholy Angel`, the flag can be obtained from Melancholy Angel's logo texture file.

![osint4](/assets/posts/wgmyctf2024/osint4.png)

![osint5](/assets/posts/wgmyctf2024/osint5.png)

> The amount of rabbit holes me and my teammate @PLZ ENTER TEXT went through was insane. We attempted to carve data from the game data files, emulate the Xbox game with Xemu, figure out how to convert the game data files into an ISO file and search online users called Melancholy Angel on social media. Safe to say, our sanity was dead.
...
We are given a DCM file to investigate. Analyzing it on CyberChef, the flag characters can be identified, however, the flag was incorrect.

![dcm1](/assets/posts/wgmyctf2024/dcm1.png)

Being confused why the flag was incorrect, my teammate @pikaroot managed to identify something suspicious. The challenge description seems to be an array with the index range of 0-31 (which is length of the flag). Arranging the characters using the appropriate index, the flag can be obtained.
...
Accessing the first website (`http://3.142.133.106:8000/`), the API seems to be expecting the first part of the flag and the correct date within the URL parameters to output the second part of the flag.
