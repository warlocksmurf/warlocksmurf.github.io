...
categories: [ctf]
tags: [blockchain,sekai]
image: /assets/posts/sekaictf2024/icon.svg
---
...
Hence, a manipulated signature can be forged by setting the v, r, s, and h values to 0. When these values are zero, the ecrecover function will return address(0), effectively making the signer's address equal to address(0). Because there is no zero-address check on results of the ecrecover function, any invalid signatures can be used to grant unauthorized permission. By converting the player address to address(0), the player essentially becomes the owner of address(0), thus having access the the ether stored in it.
