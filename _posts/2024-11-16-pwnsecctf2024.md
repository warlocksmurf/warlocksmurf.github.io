...
So the first thing was to obtain the public and private RSA keys from the beacon key file using a [script](https://github.com/Slzdude/cs-scripts/blob/master/parse_beacon_keys.py).
...
After extracting the RSA keypair, the metadata blob must first be decoded from netbiosu using the following script:
...
With the decoded metadata blob, the private RSA key can be used to obtain the metadata in cleartext. This can be done using the following script:
