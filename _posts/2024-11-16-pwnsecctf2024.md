...
This is a script that I made with reference to another [script](https://github.com/DidierStevens/DidierStevensSuite/blob/master/cs-decrypt-metadata.py) from the DidierStevensSuite.
...
The script will help extract the public and private RSA keys from the beacon key file from a beacon key file, decode the metadata blob input, and parse the metadata in cleartext using the private RSA key. The script will list down the field values according to the the Cobalt Strike metadata structure. The raw key can be identified to be `6217a4d8575c2f1e1d318292a7bb371e`
...
The script output shows the executable to be `Maintenance.exe`.
